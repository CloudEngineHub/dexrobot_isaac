...
class DexHandBase(VecTask):
    ...
    def _create_components(self):
        ...
        # Note: Removed explicit CUDA synchronization that was causing errors
    ...
    def create_sim(self):
        ...
        try:
            # Double-check device compatibility with GPU pipeline
            if self.cfg["sim"].get("use_gpu_pipeline", False) and self.device == 'cpu':
                print("WARNING: GPU Pipeline is enabled but using CPU device. This may cause errors. Disabling GPU pipeline.")
                self.cfg["sim"]["use_gpu_pipeline"] = False

            # Create simulation - use consistent approach from Isaac Gym examples
        ...
        try:
            # Create components immediately for all pipelines (matching IsaacGymEnvs approach)
            self._create_components()
            self._components_created = True
            print("Components created successfully")

            # Verify components were created if we're not using GPU pipeline
            # (With GPU pipeline, components will be created after first step)
            if not self.cfg["sim"].get("use_gpu_pipeline", False):
        ...
        print("\n[8] Preparing simulation...")
        try:
            self.gym.prepare_sim(self.sim)
            print("Simulation prepared successfully")

            # Critical: When using GPU pipeline, do NOT call simulate() immediately after prepare_sim()
            # This matches the sequence in the working gpu_pipeline_debug.py
        except Exception as e:
            print(f"Error preparing simulation: {e}")
            import traceback
            traceback.print_exc()
            raise RuntimeError("Failed to prepare simulation - cannot continue initialization") from e

        # Refresh tensors for both CPU and GPU pipelines (matching IsaacGymEnvs approach)
    ...
    def _apply_tensor_states(self, env_ids):
        ...
        # The _step_physics wrapper will automatically track this and adjust
        # physics_steps_per_control_step accordingly
        self._step_physics()
    ...
    def step(self, actions):
        ...
        # Step physics - use _step_physics wrapper
        try:
            self._step_physics()
        except Exception as e:
        ...
        # Refresh state tensors - match IsaacGymEnvs behavior
        # With GPU pipeline, refresh only happens if we're on CPU device
        try:
            # With GPU pipeline on GPU device, refreshes happen during fetch_results
            # Only refresh explicitly if on CPU or not using GPU pipeline
            if self.device == 'cpu' or not self.cfg["sim"].get("use_gpu_pipeline", False):
                self.gym.refresh_dof_state_tensor(self.sim)
                self.gym.refresh_actor_root_state_tensor(self.sim)
                self.gym.refresh_rigid_body_state_tensor(self.sim)
        except Exception as e:
        ...
        # When using GPU pipeline, create components after first successful physics step
        if hasattr(self, '_components_created') and not self._components_created:
            print("First physics step completed successfully. Creating components now...")
            try:
                self._create_components()
                self._components_created = True
                print("Components created successfully after first physics step")
            except Exception as e:
                print(f"Error creating components after first physics step: {e}")
                import traceback
                traceback.print_exc()
    ...
    def post_physics_step(self):
        ...
        if not hasattr(self, 'auto_detected_physics_steps'):
            ...
            if not hasattr(self, 'physics_step_count'):
                self.physics_step_count = 0
            if not hasattr(self, 'last_control_step_count'):
                self.last_control_step_count = 0
        ...
        # All components should already be created - this is a safety check
        if not hasattr(self, '_components_created') or not self._components_created:
            print("WARNING: Components were not created during initialization. Creating them now...")
            self._create_components()
            self._components_created = True
            print("Components created successfully during post_physics_step")
        ...
        # Update fingertip colors - only if components are created
        if hasattr(self, '_components_created') and self._components_created:
            try:
        ...
        # Process camera control - only if components are created
        if hasattr(self, '_components_created') and self._components_created:
            try:
                if hasattr(self, 'camera_controller') and self.camera_controller is not None:
                    self.camera_controller.check_keyboard_events(lambda env_ids: self.reset_idx(env_ids))
    ...
    def _step_physics(self):
        ...
        """
        # Initialize tracking variables if they don't exist yet
        if not hasattr(self, 'physics_step_count'):
            self.physics_step_count = 0
        if not hasattr(self, 'last_control_step_count'):
            self.last_control_step_count = 0
        ...
        # We track how many physics steps occur between control steps
        # and adjust physics_steps_per_control_step accordingly
        if not hasattr(self, 'auto_detected_physics_steps'):
    ...
    def pre_physics_step(self, actions):
        ...
        # Physics step count is now handled in _step_physics

        # Determine whether this is a control step or just a physics step
        # Initialize physics_step_count if not already done by _step_physics
        if not hasattr(self, 'physics_step_count'):
            self.physics_step_count = 0
        if not hasattr(self, 'last_control_step_count'):
            self.last_control_step_count = 0
