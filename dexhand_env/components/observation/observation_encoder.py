...
# Import constants
from dexhand_env.constants import NUM_BASE_DOFS, NUM_ACTIVE_FINGER_DOFS, NUM_FINGERS
...
class ObservationEncoder:
    ...
    def initialize(
        ...
        Note: Local rigid body indices (hand, fingertip, fingerpad) are
    ...
    @property
    def hand_index(self):
        """Access hand LOCAL rigid body index from parent (single source of truth)."""
        return self.parent.hand_local_rigid_body_index

    @property
    def fingertip_indices(self):
        """Access LOCAL fingertip indices from hand_initializer (single source of truth)."""
        if self.hand_initializer is None:
            raise RuntimeError("hand_initializer not set in ObservationEncoder")
        # Return local indices directly - no conversion needed
        return self.hand_initializer.fingertip_local_indices

    @property
    def fingerpad_indices(self):
        """Access LOCAL fingerpad indices from hand_initializer (single source of truth)."""
        if self.hand_initializer is None:
            raise RuntimeError("hand_initializer not set in ObservationEncoder")
        # Return local indices directly - no conversion needed
        return self.hand_initializer.fingerpad_local_indices

    @property
    def episode_time(self):
        """Access episode time from parent (single source of truth)."""
        return self.parent.episode_time

    @property
    def task(self):
        """Access task from parent (single source of truth)."""
    ...
    def _compute_active_finger_dof_indices(self) -> torch.Tensor:
        ...
        from dexhand_env.components.initialization.hand_initializer import (
            HardwareMapping,
        )
    ...
    def _compute_default_observations(self) -> Dict[str, torch.Tensor]:
        ...
        contact_binary_threshold = self.cfg["env"].get("contactBinaryThreshold", 1.0)
        contact_binary = (
    ...
    def _compute_task_observations(
        ...
        task_obs = self.task.get_task_observations(default_obs_dict)
